 ...
 * 
 * 
 * @typedef {Object} DivStyle
 * @property {Style} style
 * @property {string[][]} dividers
 * 
 * @typedef {Object} Data
 * @property {Style=} style
...
const DEFAULT_DVIDERS = [
	['┌', '┬', '┐', '─', '│'],
	['├', '┼', '┤', '─', '│'],
	['└', '┴', '┘', '─', '│']
];

const ANSII_DIVIDERS = [
	['+', '+', '+', '-', '|'],
	['+', '+', '+', '-', '|'],
	['+', '+', '+', '-', '|']
];
...
function example()
{
	...
	//if we want color we have to provide an object directy
	let data = [
		[{style: {color: 'red'}, text: 'nope'}, 1, "0x1"],
		//{ style: { color: 'red', fontWeight: 'bold', letterSpacing: spacing }, text: boldText },
		[{style: {color: {r: 0, g: 255, b: 0}, fontStyle: 'italic'}, text: 'italic text'}, 2, "0x2"],
		[{style: {color: 'white', textDecorationLine: 'line-through', textDecorationStyle: 'solid'}, text: 'strikethrough text'}, 3, "0x3"]
	];
	...
	// width : Width of the column content //override min and maxWidth
	// maxWidth: content is truncated to this length
	// minWidth: columns are this length at minimum
	// color
	// pad   : 0 for center, < 0 for left, > 0 for right
	const columns = [
		{header: 'Test', width: 10},
...
function printTable(data, columns = undefined, style = defaultStyle(), constraints = undefined)
{
	...
	for (let i = 0; i < data[0].length; i++)
	{
		if (i >= columns.length)
		{
			columns.push({header: '', padHeader: 0, padContent: 0});
		}
		if (columns[i].width == undefined)
		{
			...
			for (let j = 0; j < data.length; j++)
			{
				...
				let len = 0;
				if (data[j][i] instanceof Array)
				{
					len = data[j][i].map(d => d == null ? 0 : d.text != undefined ? d.text.length : d.toString().length).reduce((a, b) => a + b, 0);
				} else
				{
					len = data[j][i].text != undefined ? data[j][i].text.length : data[j][i].toString().length;
				}
				if (len > longest)
				{
					longest = len;
...
function printBorder(header, borderIndex, style, constraints)
{
	...
	{
		printStack.push({style: {color: style.style.color}, text: ''.padEnd(header[c].width, style.dividers[borderIndex][FILLER])});
		if (c == header.length - 1)
		{
			printStack.push({style: {color: style.style.color}, text: style.dividers[borderIndex][CLOSER]});
		} else
		{
...
function colorPrint(stack, colorType = COLOR_FORMAT.TRUE, constraints)
{
	...
	for (let i = 0; i < stack.length; i++)
	{
		...
		{
			style = {style: {color: stack[i].style ? stack[i].style.color || stack[i].style : stack[i].color, backgroundColor: '#000000'}};
		}
...
function colorToAnsiiCode(color, bg = false, colorFormat = COLOR_FORMAT.TRUE)
{
	...
	if (colorFormat == COLOR_FORMAT.TRUE)
	{
		...
		{
			color = rgbFromHtml(color);
		} else
		{
			let match = COLORS.find(c => c.desc.toLowerCase() == color.toLowerCase);
			if (!match)
			{
				match = COLORS.find(c => c.desc.toLowerCase().startsWith(color.toLowerCase()));
			}
			if (match)
			{
				match = rgbFromHtml(match.html);
			} else
			{
				match = rgbFromHtml("#FFFFFF"); //white
			}
			color = match;
		}
		if (bg)
	...
	if (!match)
	{
		match = colors.find(c => c.html == "#ffffff");
	}
	color = match.ansi;
	switch (colorFormat)
	{
		...
		case COLOR_FORMAT[16]:
			...
			if (color > 8)
			{
				if (colorFormat == COLOR_FORMAT[16])
				{
					bright = true;
				}
				color -= 8;
...
// This functions finds the closest match to a full range HTML color code in the 256 color ANSI colors
function findHtmlColorEquivalent(htmlCode, colors = COLORS)
{
...
{
	let r = Number.parseInt('0x' + htmlCode.substring(1, 3));
